...
function StockRow({ stock, color, canRemove, onRemove }) {
  ...
  return (
    <div className="flex items-center justify-between gap-2 group">
      <div className="flex items-center gap-2 shrink-0 min-w-0">
        ...
        <span className="font-optima font-bold text-sm uppercase tracking-wide truncate" style={{ color: 'rgba(200,200,230,0.75)' }}>
          {stock.label}
        </span>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <span className="font-optima font-semibold text-base" style={{ color: 'rgba(235,235,255,0.92)' }}>{priceStr}</span>
        <span className="font-optima text-sm font-medium w-20 text-right" style={{ color: positive ? '#4ade80' : '#f87171' }}>{changeStr}</span>
        {canRemove && (
          <button
